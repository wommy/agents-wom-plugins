...
const toolDefinitions = [
	{ name: 'observe_l1_event', description: 'Observe a Claude task or Codex native-plan lifecycle event.', inputSchema: { type: 'object', required: ['client', 'event'], properties: { client: { type: 'string', enum: ['claude', 'codex'] }, event: { type: 'object' } } } },
	{ name: 'tudo_status', description: 'Read the durable L1 mirror projection.', inputSchema: { type: 'object', properties: {} } },
	{ name: 'tudo_promote', description: 'Report unfinished mirror rows absent from a client-owned L1 plan; never creates or deletes tasks.', inputSchema: { type: 'object', properties: { have: { type: 'array', items: { type: 'string' } } } } },
	{ name: 'tudo_link', description: 'Link an existing L1 mirror row to an existing durable L2 backlog identity after validating the host roadmap.', inputSchema: { type: 'object', required: ['id', 'n', 'repo'], properties: { id: { type: 'string' }, n: { type: 'integer' }, repo: { type: 'string' }, roadmap: { type: 'string' } } } },
	{ name: 'tudo_demote', description: 'Plan L1-to-L2 demotion by default; apply writes via the host Womr TOON rail, then links mirror rows.', inputSchema: { type: 'object', properties: { repo: { type: 'string' }, roadmap: { type: 'string' }, apply: { type: 'boolean', default: false } } } },
	{ name: 'tudo_add', description: 'Agent-side row insertion into the L1 mirror.', inputSchema: { type: 'object', required: ['id', 'subject'], properties: { id: { type: 'string' }, subject: { type: 'string' }, description: { type: 'string' }, backlogN: { type: 'integer', minimum: 1 }, sessionId: { type: 'string' } } } },
] as const
